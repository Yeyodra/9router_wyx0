...
async function driveGoogleAuth(page, email, password, { onStep, timeout = 90_000 } = {}) {
  ...
  for (let i = 0; i < 180; i++) {
    ...
    // ── detect Google challenge / verification pages that cannot be automated
    //    (phone verify, "Verify it's you", image CAPTCHA, suspicious activity)
    //    Route to manual assist immediately instead of burning the full timeout.
    const challengeSelectors = [
      // Phone / SMS verification
      'input[type="tel"]',
      'input[name="phoneNumberId"]',
      // "Verify it's you" / recovery email prompts
      '[data-challengetype="12"]',
      '[data-challengetype="10"]',
      '[data-challengetype="13"]',
      // reCAPTCHA iframe
      'iframe[src*="recaptcha"]',
      'iframe[src*="captcha"]',
      // "This account doesn't exist" / "Couldn't sign you in"
      ...
      'h1:has-text("Confirm it\'s you")',
    ].join(", ");

    const challengeLoc = page.locator(challengeSelectors).first();
    const challengeVisible = await challengeLoc.isVisible({ timeout: 300 }).catch(() => false);
    if (challengeVisible) {
      throw Object.assign(
        new Error("Google challenge detected (phone verify / CAPTCHA / suspicious activity) — requires manual assist"),
        { step: "google_challenge_detected" }
      );
    }
...
export class QwenCloudBulkImportManager extends KiroBulkImportManager {
  ...
  async processAccount(job, account, workerId, browser) {
    ...
    try {
      ...
      // Google challenge detected → route to manual assist instead of hard fail
      if (error.step === "google_challenge_detected") {
        this.setAccountStep(account, "manual_assist", error.message);
        account.status = "manual_assist";
        account.error = error.message;
        account.manualSession = { context, page };
        account.runtimeSession = null;
        await this.persistJobSnapshot(job, { forcePreview: true });
        // Do NOT close context — keep browser open for manual intervention
        return;
      }
